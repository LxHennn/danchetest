...
uint8 icm20602_init (void)
{
    ...
    do
    {
        ...
        icm20602_write_register(ICM20602_SMPLRT_DIV,     0x01);                 // 采样速率 SAMPLE_RATE = INTERNAL_SAMPLE_RATE / (1 + SMPLRT_DIV)
...
void lingpiao(void)
{
    if(lp<100)
    {
				if(icm20602_gyro_transition(icm20602_gyro_x)<5&&icm20602_gyro_transition(icm20602_gyro_x)>-5)
				{
					err[0]+=icm20602_gyro_transition(icm20602_gyro_x);
					err[1]+=icm20602_gyro_transition(icm20602_gyro_y);
					err[2]+=icm20602_gyro_transition(icm20602_gyro_z);
					lp++;
				}
		}
		else if(lp==100)
...
float High_Flilter(float indata,Flilter* a)
{
    float data = 0;
    float RC,K;
		a->Cutoff=5000;
		a->Fs=200;
    RC = (float)1/2/PI/a ->Cutoff;
    K = 1/a->Fs/(RC + 1/a->Fs);
    data = K*( a->Last_data +  indata - a->Last_inital_data );
    a->Last_inital_data = indata;
    a->Last_data = data;
    return data;
}

float Low_Flilter1(float indata,Flilter* a)
{
    float data = 0;
    float RC,K;
	a->Cutoff=5;
...
float Low_Flilter2(float indata,Flilter* a)
{
    float data = 0;
    float RC,K;
	a->Cutoff=10;
...
Flilter accx,accy,accz;
Flilter gyrox,gyroy,gyroz;
float accp,accr;
int flag=0;
void Angle_acc_cal(void)
{
	float acx,acy,acz;
    /* accx->Cutoff=500;
    accy->Cutoff=500;
    ...
    accy->Fs=250;
    accz->Fs=250; */
	acx=Low_Flilter2(icm20602_acc_transition(icm20602_acc_x),&accx);
	acy=Low_Flilter2(icm20602_acc_transition(icm20602_acc_y),&accy);
	acz=Low_Flilter2(icm20602_acc_transition(icm20602_acc_z),&accz);

    //ptf("%f%f",acz,(float)icm20602_acc_z);
//	if(acz>0)
...
//	{
//		if(acy>0)
//			accr = 180-atan(acy/sqrt(acx*acx+acz*acz))*180/PI;
//		else
//			accr= -180-atan(acy/sqrt(acx*acx+acz*acz))*180/PI;
//	}
//		else
