...
void servobalance()
{
	...
	pwm_set_duty(SERVO_MOTOR_PWM,SERVO_MOTOR_DUTY(92.5-duoji));
}
...
int main(void)
{
  ...
  flash_init();
	...
	pwm_init(SERVO_MOTOR_PWM, SERVO_MOTOR_FREQ, SERVO_MOTOR_DUTY(92.5));
	...
	while (1)
	{	
		data_len = wireless_uart_read_buffer(data_buffer, 1);                    // 查看是否有消息 默认缓冲区是 WIRELESS_UART_BUFFER_SIZE 总共 64 字节
...
void pit_handler0(void)
{
	...
	angle_z += ((gyro_z + 0.128401913) * 0.001);
	if (angle_z > 360)
		angle_z -= 360;
	if (angle_z < 0)
		angle_z += 360;
...
void pit_handler2(void)
{
	...
	if(encoder2>run+3.5&&xiapo)pos6.ek_sum=0;
...
//	if(angle-e>8||angle-e<-8)pos3.ek_sumlimit=1000;
//	else if(angle-e>5||angle-e<-5)pos3.ek_sumlimit=2000;
//	else if(angle-e>3||angle-e<-3)pos3.ek_sumlimit=3000;
//	else if(angle-e>1||angle-e<-1)pos3.ek_sumlimit=4000;
//	else pos3.ek_sumlimit=5000;
}

float distance;
void pit_handler3(void)
{
	angle_z -= 0.0286788025;
	if (gps_tau1201_flag)
	{
		gps_tau1201_flag = 0;
		if (!gps_data_parse()) // 开始解析数据
		...
		if(key1==500)
		{
			if (gps_tau1201_flag)
			{
				...
				if (!gps_data_parse()) // 开始解析数据
				{
					...
					((double*)&(flash_union_buffer[i].uint32_type))[0]=caidian[i];		
          ((double*)&(flash_union_buffer[j].uint32_type))[0]=caidian[j];	
					
					end=(j+1)/2;
					for(c=0;c<30;c++)
					ptf("%lf%lf%d%d%d",caidian[i],caidian[j],i,j,end);
					i+=2;
					j+=2;
				}
			}
     ...
     if (flash_check(FLASH_SECTION_INDEX, FLASH_PAGE_INDEX)) // 判断是否有数据
	  {
		...
		//((double*)&(flash_union_buffer[0].uint32_type))[0]=37.2345;
		flash_write_page_from_buffer(FLASH_SECTION_INDEX, FLASH_PAGE_INDEX);
		flash_buffer_clear();
		for(c=0;c<30;c++)
			ptf("%d",1);
...
void key4_exti_handler(void)
{
	if (exti_flag_get(KEY4))
	{
      ...
      flash_read_page_to_buffer(FLASH_SECTION_INDEX, FLASH_PAGE_INDEX);
			for(c=0;c<end;c++){
				  caidian[i]=((double*)&(flash_union_buffer[i].uint32_type))[0];		
          caidian[j]=((double*)&(flash_union_buffer[j].uint32_type))[0];	
				  i+=2;
				  j+=2;
			}
			i=i-2;
